...
function PlayerTable({index,name,number}) {
    ...
    function handleClick() {
        if(number > -1 && number <101 && number) { 
            setIsShow(pv=>!pv) 
        }else alert('Enter 0 to 100 number')
    }
  ...
  return (
    <div className='p-5 bg-green-600 rounded w-1/4 text-start'>
    ...
    <button className="btn bg-gray-600 p-1 mt-2 rounded" onClick={handleClick}>{isShow?'Hide':'Show'}</button>
  </div>
  )
}
