...
function Form() {
    ...
    // useEffect(() => {
    //         searchParams.get('userId') !== null ?  getUserId()  : setObjForEdit(location.state)
    // }, []);
  ...
  return (
    <div className='bg-blue-950 d-flex flex-col' style={{height:'100vh',alignItems:'center'}}>
        <Link to='/' className='m-1 bg-warning p-2 w-fit rounded text-black text-decoration-none'>Home</Link>
      <form className='container w-50 py-4 bg-white m-2 rounded px-4' onSubmit={handleSubmit}>
        <div className='row p-2'>
            <label htmlFor='firstname'>Firstname</label>
            <input required type="text" defaultValue={objOfData.firstname} onChange={(e)=>{handleOnChange(e)}} id='firstname'/>
            <p className='text-danger m-0'>{objOfValidation.firstname}</p>
        </div>
        <div className='row p-2'>
            <label htmlFor='lastname'>Lastname</label>
            <input required type="text" defaultValue={objOfData.lastname} onChange={(e)=>{handleOnChange(e)}}  id='lastname'/>
            <p className='text-danger m-0'>{objOfValidation.lastname}</p>
        </div>
        <div className='row p-2'>
            <label htmlFor='email'>Email</label>
            <input required type="email" defaultValue={objOfData.email} onChange={(e)=>{handleOnChange(e)}}  id='email'/>
            <p className='text-danger m-0'>{objOfValidation.email}</p>
        </div>
        <div className='row p-2'>
            <label htmlFor='phone'>Phone</label>
            <input required type="number" defaultValue={objOfData.phone} onChange={(e)=>{handleOnChange(e)}} id='phone'/>
            <p className='text-danger m-0'>{objOfValidation.phone}</p>
        </div>
        <div className='row p-2'>
            <label htmlFor='password'>Password</label>
            <div className='password-row p-0'>
                <input required type={isPasswordShow?'text':'password'} className='pl-3 border-none' style={{width:'95%'}} defaultValue={objOfData.password} onChange={(e)=>{handleOnChange(e)}} id='password'/>
                <span className='cursor-pointer' onClick={()=>{setIsPasswordShow(pv=>!pv)}}>{isPasswordShow?'😐':'😑'}</span>
            </div>
            <p className='text-danger m-0'>{objOfValidation.password}</p>
        </div>
        <div className='row d-flex  p-2 my-3 pb-3'>
            <input type="submit" value={useId?'Update':'Submit'} className='w-25 m-1 border-none bg-primary' />
            <button className="btn m-1  w-25 bg-primary" onClick={()=>{navigate('/')}}>Cancel</button>
        </div>
      </form>
    </div>
  )
